...
void updateGrblFilesBtn ( void ) {  // fill 4 buttons for file name
    ...
    if (grblFileIdx ) { // fill buttons only if there are name in the array
...
/*
// look at sd card
boolean _sdStart( void ) { // this function is called when we enter the sd screen or when we navigate between files with Prev/Next buttons
